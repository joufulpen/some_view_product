...
var resourceName = 'Part' //资源名称 模板生成
...
window.vm = new Vue({
  ...
  computed: {
    options() {
      return Object.keys(this.searchConditions)
        .map(key => ({ key, value: this.allData.map(part => part[key]).filter((v, i, a) => a.indexOf(v) === i) }))
        .reduce((acc, cur) => {
  ...
  methods: {
    customSave() {
      axiosDict[apiName].post(this.saveURL, {parentId: id, items: this.allData}).then(res => {
        console.log(res)
        if (res) {
          this.allData = res
    ...
    onSuccess(response, file, fileList) {
      if (response.state == 'success') {
        this.allData = response.rows //this.allData.filter(r => r.type == '玻璃').concat(response.rows)
        this.currentPage = 1 //Math.ceil(this.allData.length / this.pageSize)
    ...
    search() {
      ...
      if (window.location.hash.split('#')[4] == 'apply') {
        let condition = [
          { FieldName: 'ParentID', TableName: '[Part]', Value: [{ value: id }], TableRelationMode: 'AND', Mode: '等于', DataType: 'string' },
          {
            FieldName: 'type',
            TableName: '[Part]',
            Value: [{ value: '玻璃' }, { value: '铝板' }, { value: '折弯铝板' }],
      ...
      axiosDict[apiName]
        ...
        .then(res => {
          ...
          if (res) {
            this.allData = res
            // this.$refs.singleView.singleTableData.data = res.slice(0,200)
            // this.showData(this.allData, this.pageSize, this.currentPage)
